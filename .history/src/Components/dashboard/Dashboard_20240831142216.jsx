...
const Dashboard = () => {
  const { SideBar و } = useSelector((state) => state.sharedStore);
  const { user, token } = useSelector((state) => state.auth);
  const { msg, toggel } = useSelector((state) => state.task.status);
const showNotifcationTask = (ele) => {
  ...
  return (
    <div>
      ...
      <OutletDashboard />
      {toggel ? <ToastModal msg={msg} /> : null}
    </div>
  );
};
