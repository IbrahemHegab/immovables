...
import { setOpenSideBar, setTaskID } from "../store/SharedStore";
...
const Dashboard = () => {
  ...
  const showNotifcationTask = (ele) => {
    dispatch(
      ...
      navigate("employees/myTask");
      dispatch(setTaskID(ele.task));
    });
  };
                ...
                <div className="flex relative">
                    ...
                    {user
                      ? user.notifications.map((e, inx) => (
                          ...
                          >
                            ...
                            {`تم استلام مهمه جديده من:- ${e.assignedBy || e.assignedTo}`}
                          </Dropdown.Item>
                        ))
                      : ""}
                  ...
                  <span
                    className={`absolute top-0  ${
                      user?.notifications ? "bg-green-600" : "bg-red-600"
                    } rounded-full w-5 h-5 text-gray-50 flex items-center justify-center text-xs`}
                    style={{ right: "10px" }}
                  >
