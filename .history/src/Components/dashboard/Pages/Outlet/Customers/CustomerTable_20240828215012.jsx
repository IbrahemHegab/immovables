  ...
  const columns = useMemo(
    () => [
      ...
      // },
      {
        ...
        size: 100,
        Cell: ({ row }) => (
          <div className="grid grid-cols-2  gap-2 bg-danger">
            <MainBtn btn={<MdEdit onClick={() => handleEditClick(row)} />} className="bg-danger"/>

            <MainBtn
              btn={<MdDeleteForever onClick={() => handleDeleteClick(row)} className="bg-danger"/>}
            />
          </div>
        ),
      },
