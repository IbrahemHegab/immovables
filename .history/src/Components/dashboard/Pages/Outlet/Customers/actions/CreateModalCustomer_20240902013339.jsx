...
const CreateModalCustomer = () => {
  ...
  const sharedStore = useSelector((state) => state.auth);
  const auth = useSelector((state) => state.sharedStore);
  const dispatch = useDispatch();

  const toggleMarketer = (marketer) => {
    setSelectedMarketers((prevSelected) => {
      const isSelected = prevSelected.some((item) => item._id === marketer._id);
  
      if (isSelected) {
        // إزالة الـ marketer من القائمة إذا كان موجودًا
        return prevSelected.filter((item) => item._id !== marketer._id);
      } else {
        // إضافة الـ marketer إلى القائمة إذا لم يكن موجودًا
        return [...prevSelected, marketer];
      }
    });
  };
                      ...
                      employees
                        ...
                        .map((marketer) => (
                          ...
                          >
                            <input
                              type="checkbox"
                              defaultChecked={selectedMarketers.some(
