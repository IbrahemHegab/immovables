...
const UpdateModalCustomer = () => {
  ...
  const [selectedMarketers, setSelectedMarketers] = useState(
    initstate.marketers.length > 0 ? 
    initstate.marketers :
    UpdateCustomerModel.data?.marketers
  );
console.log(selectedMarketers);

  // const marketers = ["اسلام", "محمد", "احمد"];
              ...
              <div className="col-span-1">
                ...
                <div className="flex justify-center text-center bg-gray-200 p-1 rounded-lg">
                  {/* <Dropdown label={getLabel()} inline>
                    {UpdateCustomerModel.data?.marketers.map((marketer) => (
                      <Dropdown.Item
                        key={marketer}
                    ...
                    {employees.data.length > 0 && employees.data ? (
                      employees.data.map((marketer) => (
                        ...
                        >
                          <input
                            type="checkbox"
                            checked={selectedMarketers?selectedMarketers.includes(marketer.name):""}
                            readOnly
                            className="mr-2"
                          />
                          {marketer.name}
