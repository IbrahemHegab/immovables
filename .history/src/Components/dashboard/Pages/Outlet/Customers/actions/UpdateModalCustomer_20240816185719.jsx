...
console.log(selectedMarketers);
  ...
  const toggleMarketer = (marketer) => {
    ...
    setSelectedMarketers((prevSelected) =>
      UpdateCustomerModel?.data.marketers.includes(marketer)
        ? UpdateCustomerModel?.data.marketers.filter((item) => item !== marketer)
        : [...UpdateCustomerModel?.data.marketers, marketer]
    );
  };
