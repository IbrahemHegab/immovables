...
const UpdateModalCustomer = () => {
  ...
  const toggleMarketer = (marketer) => {
    console.log(import { Button, Modal, Label, Dropdown } from "flowbite-react";
      import { useDispatch, useSelector } from "react-redux";
      import { setOpenCustomerModelUpdate } from "../../../../../store/SharedStore";
      import {
        getCustomer,
      ...
      const UpdateModalCustomer = () => {
        ...
        const [selectedMarketers, setSelectedMarketers] = useState(initstate.marketers);
        ...
        const getLabel = () => {
          return selectedMarketers.length === 0 ? "المسوقيين" : selectedMarketers.join(", ");
        };
                            ...
                            employees.data.map((marketer, inx) => (
                              ...
                              >
                                <input
                                  ...
                                  onChange={() => toggleMarketer(marketer.name)}
                                  className="mr-2"
                                />
                                {marketer.name}
                              </Dropdown.Item>
  ...
  const getLabel = () => {
    if (UpdateCustomerModel.data?.marketers.length === 0) {
      return "المسوقيين";
    }
    return UpdateCustomerModel.data?.marketers.join(", ");
  };
  const dispatch = useDispatch();
  const submit = (values) => {
    values.marketers = selectedMarketers;
                    ...
                    {employees.data.length > 0 && employees.data ? (
                      employees.data.map((marketer, inx) => (
                        ...
                        >
                          <input
                            type="checkbox"
                            checked={UpdateCustomerModel.data?.marketers.includes(
                              marketer.name
                            )}
                            readOnly
                            className="mr-2"
