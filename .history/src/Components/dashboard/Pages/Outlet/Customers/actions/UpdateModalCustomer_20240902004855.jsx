...
const UpdateModalCustomer = () => {
  ...
  console.log(initstate.marketers);
  ...
  {initstate.marketers.length > 0 && initstate.marketers ? (
    initstate.marketers.map((marketer, inx) => (
      ...
      >
        <input
          type="checkbox"
          checked={selectedMarketers
            .map((ele) => ele.name)
            .includes(marketer.name)} // تحقق إذا كان المسوق موجودًا في selectedMarketers
          onChange={() => toggleMarketer(marketer.name)} // استدعاء toggleMarketer لتحديث القائمة عند النقر على الـ checkbox
          className="mr-2"
        />
        {marketer.name}
      </Dropdown.Item>
