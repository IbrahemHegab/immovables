...
const UpdateModalCustomer = () => {
  ...
  const toggleMarketer = (marketer) => {
    ...
    setSelectedMarketers((prevSelected) =>
      prevSelected.includes(marketer)
        ...
        : [...prevSelected, marketer وmarketersFromEmployees]
    );
  };
  ...
  {filter === "marketers" ? (
    ...
    employees.data.length > 0 && employees.data ? (
      employees.data
        .filter((ele) => ele.role === "marketer") // استخدم role للتحقق من كونهم مسوقين
        .map((marketer, inx) => (
          <Dropdown.Item key={inx}>
            <input
              type="checkbox"
    ...
    <Dropdown.Item>اختر نوع البيانات</Dropdown.Item>
  )}
</Dropdown>
