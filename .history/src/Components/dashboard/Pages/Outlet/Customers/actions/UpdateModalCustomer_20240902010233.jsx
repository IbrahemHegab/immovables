...
const UpdateModalCustomer = () => {
  ...
  const toggleMarketer = (marketer) => {
    const marketersFromEmployees = employees.data.filter(
                    ...
                    {initstate.marketers.length > 0 && initstate.marketers ? (
                      initstate.marketers.map((marketer, inx) => (
                        <Dropdown.Item key={inx}>
                          <input
                            type="checkbox"
                            checked={selectedMarketers.includes(...marketer ,employees.data.filter(
    (employee) => employee.role === "marketer"
  ))} // تحقق مما إذا كان المسوق موجودًا في selectedMarketers
