...
const UpdateModalCustomer = () => {
  ...
  // Merge employees who are marketers into the initial state marketers list
  const marketersFromEmployees = employees.filter(
    (employee) => employee.role === "marketer"
  );
                ...
                <div className="flex justify-center text-center bg-gray-200 p-1 rounded-lg">
                  <Dropdown label={getLabel()} inline>
                    {marketersFromEmployees.length > 0 ? (
                      marketersFromEmployees.map((marketer, inx) => (
                        //console.log(selectedMarketers)
                        
                        <Dropdown.Item key={inx}>
                          <input
                            type="checkbox"
                            checked={selectedMarketers.map((e)=>e.name).includes(marketer.name)}
