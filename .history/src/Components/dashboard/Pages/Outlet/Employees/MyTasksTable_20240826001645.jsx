...
const TasksTable = () => {
  ...
  const filteredData = task.data.filter((e) => {
    ...
    return e.assignedTo._id === filter , e.assignedTo._id === filter;
  });
console.log(filter);
  ...
  return (
    <div className="overflow-x-scroll xl:w-auto lg:w-auto w-screen h-screen">
      <div className="flex justify-center gap-3 items-center py-2">
        ...
        <Button onClick={() => setFilter(auth.user?._id)}>المهام المسنده الي</Button>
        <Button onClick={() => setFilter(auth.user?._id)} >المهام المسنده اليهم</Button>
      </div>
      <Table dir="rtl">
