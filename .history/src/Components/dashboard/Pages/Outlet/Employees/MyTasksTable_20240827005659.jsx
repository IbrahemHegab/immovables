...
const TasksTable = () => {
  ...
  const [loadingAssignedTasks, setLoadingAssignedTasks] = useState(false); // حالة لتحميل البيانات المسندة
  ...
  useEffect(() => {
    ...
    if (showAssignedTasks) {
      setLoadingAssignedTasks(true); // تحديد حالة التحميل
      dispatch(getMyTaskAssignedTo(auth.token))
        .finally(() => setLoadingAssignedTasks(false)); // إلغاء التحميل بعد الجلب
    } else {
  ...
  return (
    <div className="overflow-x-scroll xl:w-auto lg:w-auto w-screen h-screen">
      <div className="flex justify-center gap-3 items-center py-2">
        <Button
          onClick={() => {
            setShowAssignedTasks(false); // جلب جميع المهام
        ...
        <Button
          onClick={() => {
            setShowAssignedTasks(true); // جلب المهام المسندة فقط
          }}
        >
          المهام المسنده الي
        </Button>
      ...
      <Table dir="rtl">
        ...
        <Table.Body className="divide-y text-center">
          {loadingAssignedTasks ? (
            <Table.Row>
              <Table.Cell colSpan="7">جاري تحميل البيانات...</Table.Cell>
            </Table.Row>
