...
const TasksTable = () => {
  ...
  const getRowColor = (diffInDays, taskDuration) => {
    console.log(diffInDays ,taskDuration);
  ...
  return (
    <div className="overflow-x-scroll xl:w-auto lg:w-auto w-screen h-screen">
      <div className="flex justify-center gap-3 items-center py-2">
        ...
        <Button className="success" onClick={() => dispatch(getMyTaskAssignedTo())}>
    المكتملة
        </Button>
        <Button color="warning" onClick={() => dispatch(getMyTaskAssignedTo())}>
          قيد المراجعة
        </Button>
        <Button color={"red"} onClick={() => dispatch(getMyTaskAssignedTo())}>
        المرفوضة
        </Button>
      </div>
      <Table dir="rtl">
        ...
        <Table.Body className="divide-y text-center">
          {task.isLoading ? (
            ...
            task.data.map((e, index) => (
              <Table.Row
              className={`dark:border-gray-700 dark:bg-gray-800 ${getRowColor(dates[index]?.diffInDays, e.taskDuration)}`}
              key={e?._id}
            >
