...
const TasksTable = () => {
  ...
  return (
    <>
      {" "}
      <div className={`grid ${auth.user.role !== "manager" ? "lg:grid-cols-6" :"lg:grid-cols-5"}  content-center grid-cols-2 gap-3 items-center py-2`}>
        <Button onClick={() => dispatch(getMyTask(auth.token))}>
          كل مهام الموظفين
        </Button>
       {auth.user.role !== "manager" &&  <Button onClick={() => filters()}>المهام المسنده الي</Button>}
