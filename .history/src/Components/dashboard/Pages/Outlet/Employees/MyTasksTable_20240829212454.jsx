...
const TasksTable = () => {
  ...
  return (
    <>
      ...
      <div
        className={`grid ${
          auth.user?.role !== "manager" ? "lg:grid-cols-6 sm:grid-cols-2"  : "lg:grid-cols-5 sm:grid-cols-2"
        }  content-center grid-cols-2 gap-3 items-center py-2`}
      >
        <Button onClick={() => filters()}>كل مهام </Button>
        {auth.user?.role !== "manager" && (
          <Button onClick={() => filters()}>مهامي</Button>
        )}
              ...
              filteredData.map((e, index) => (
                ...
                {`فرق الأيام: ${dates[index]?.diffInDays} يوم`} */}
                  ...
                  <Table.Cell style={{ borderRadius: 0 }}>
                    <div className="grid xlg:grid-cols-2  grid-cols-1 gap-2">
                      {" "}
