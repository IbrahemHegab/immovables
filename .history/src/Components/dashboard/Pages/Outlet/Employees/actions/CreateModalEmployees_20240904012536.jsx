...
const CreateModalEmployees = () => {
  ...
  const { EmployeesModel } = useSelector((state) => state.sharedStore);
  const dispatch = useDispatch();

  const submit = (values) => {
    values.role = selectedValueKind === "مشرف" ? "supervisor" :selectedValueKind === "محاسب" ? "accountant" :selectedValueKind === "مدير" ? "manager" :"marketer" ;
    
    dispatch(createEmployees(values)).then(() => {
      ...
      <Formik initialValues={initstate} onSubmit={submit}>
        {({ setFieldValue }) => (
          <Form>
            ...
            ></Modal.Header>
            <Modal.Body>
              <div className="grid grid-flow-row-dense lg:grid-cols-2 grid-cols-1 grid-rows-3 gap-3">
                <div>
                ...
                <div>
                  <div className="mb-2 block">
                    <Label htmlFor="base" value="رقم التليفون" />
                  </div>
                  <Field
                    className="text-gray-900 text-sm p-2 bg-gray-50 rounded-lg w-full border-gray-300"
                    id="base"
...
export default CreateModalEmployees;
