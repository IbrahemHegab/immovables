...
const UpdateModalEmployees = () => {
  const {  sharedStore ,auth} = useSelector((state) => state.auth);
  const {  sharedStore ,auth} = useSelector((state) => state.sharedStore);
  const initstate = {
    name:
      sharedStore.UpdateEmployeesModel.data && sharedStore.UpdateEmployeesModel.data.name
        ? sharedStore.UpdateEmployeesModel.data.name
        : "",
    jobTitle:
      sharedStore.UpdateEmployeesModel.data && sharedStore.UpdateEmployeesModel.data.jobTitle
        ? sharedStore.UpdateEmployeesModel.data.jobTitle
        : "مشرف",
    ID:
      sharedStore.UpdateEmployeesModel.data && sharedStore.UpdateEmployeesModel.data._id
  ...
  const submit = (values) => {
    ...
    .then(() => {
      dispatch(getEmployees({token:auth.token}));
    });
