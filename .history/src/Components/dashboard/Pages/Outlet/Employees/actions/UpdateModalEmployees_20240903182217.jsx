...
const UpdateModalEmployees = () => {
  ...
  const initstate = {
    ...
    role: data && data.role ? data.role : "مشرف",
    id: data && data._id ? data._id : null,
  };
  const [selectedValueKind, setSelectedValueKind] = useState(data?.role);
              ...
              <div className="grid grid-flow-row-dense lg:grid-cols-3 grid-cols-1 grid-rows-3 gap-3">
                ...
                <div className="col-span-2">
                  ...
                  <div className="flex justify-center text-center bg-gray-200 p-2 rounded-lg">
                    <Dropdown
                      label={
                        selectedValueKind || selectedValueKind?.role === "manager"
                          ? "مدير"
                          : selectedValueKind?.role === "accountant"
                          ? "محاسب"
                          : selectedValueKind?.role === "supervisor"
                          ? "مشرف"
                          : "مسوق"
                      }
                      inline
