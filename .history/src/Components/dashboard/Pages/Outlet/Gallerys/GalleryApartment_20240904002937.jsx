...
import { deleteEmployees } from "../../../../store/Employees/employeesSlice";
...
const GalleryApartment = () => {
  ...
  // };
  const handleDelete = (id) => {
    fireSwal("هل أنت متأكد من حذف الوحدة ؟", "", async () => {
      await dispatch(deleteGallery(id));
      // بدلاً من جلب البيانات مرة أخرى، يمكن تحديث الحالة مباشرةً
      dispatch(getGallery());
    });
  };
      ...
      {data.length > 0 && data ? (
        data.map((e, index) => (
          <Card className="max-w-sm card-gallery bg-bla" key={index}>
            ...
            <div className="grid grid-cols-3 content-around text-gray-900 text-center p-3 xl:mt-20 mt-20 md:mt-0">
              <div>
                <h6>المساحه</h6>
        ...
        <h2 className="m-5 flex justify-center col-span-12 text-black">
          لايوجد وحدات مشابه
        </h2>
      )}
