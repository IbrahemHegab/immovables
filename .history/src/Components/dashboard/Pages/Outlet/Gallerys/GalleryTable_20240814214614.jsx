...
const GalleryTable = () => {
  ...
  const filteredData = data.filter((e) => {
    ...
    if (filter === "villa-200-295") return e.kind === "فيلا" && e.area >= 200 && e.area <= 295;
    if (filter === "villa-300-450") return e.kind === "فيلا" && e.area >= 300 && e.area <= 450;
    return false;
  });
  return (
    <div className="">
      <div className="grid grid-cols-3 gap-5 ">
        <div className=" p-2 bg-yellow-700 rounded-xl text-center" onClick={() => setFilter("apartment")}>
          <span>الشقق</span>
        </div>
        <div className=" p-2 bg-yellow-700 rounded-xl text-center" onClick={() => setFilter("villa-200-295")}>
          <span>فيلا 200~295</span> 
        </div>
        <div className=" p-2 bg-yellow-700 rounded-xl text-center"   onClick={() => setFilter("villa-300-450")}>
