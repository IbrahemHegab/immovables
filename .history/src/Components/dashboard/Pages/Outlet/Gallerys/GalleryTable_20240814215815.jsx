...
const GalleryTable = () => {
  ...
  // تصفية البيانات بناءً على قيمة الحالة
  const filteredData = data.filter((e) => {
    if (filter === "all") return true;
    return e.kind === filter; // تأكد من أن الحقل 'kind' يحتوي على قيمة الحالة المطلوبة
  });
  ...
  return (
    <div className="">
      ...
      <div
          ...
          onClick={() => setFilter("all")} // اضبط الفلتر ليعرض الكل
        >
          <span>عرض الكل</span>
        </div>
        <div
          className="p-2 bg-yellow-700 rounded-xl text-center cursor-pointer"
          onClick={() => setFilter("شقه")} // اضبط الفلتر ليكون "شقه"
        >
          <span>الشقق</span>
        </div>
        <div
       className="p-2 bg-yellow-700 hover:bg-yellow-500 rounded-xl text-center cursor-pointer"
          onClick={() => setFilter("villa-200-295")} // اضبط الفلتر ليكون "فيلا"
        >
          <span>فيلا 200~295</span>
        </div>
        <div
          className="p-2 bg-yellow-700 hover:bg-yellow-500 rounded-xl text-center cursor-pointer"
          onClick={() => setFilter("villa-300-450")} // اضبط الفلتر ليكون "فيلا"
        >
