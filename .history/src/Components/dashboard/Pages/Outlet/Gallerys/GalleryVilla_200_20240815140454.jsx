"use client";
import { Card, Carousel, ToggleSwitch } from "flowbite-react";
...
const GalleryVilla_200 = () => {
  ...
  const handleDelete = (id) => {
    fireSwal("هل أنت متأكد من حذف العميل؟", "", async () => {
      ...
      dispatch(getGallery("villa-200-295")); // إعادة جلب البيانات بعد الحذف
    });
  };
  ...
  return (
    <div className="grid lg:grid-cols-3  sm:grid-cols-1 gap-3">
      {data.length > 0 && data
        ? data.map((e) => (
            <Card className="max-w-sm" key={e?._id}>
              <div className="h-44 sm:h-64 xl:h-48 2xl:h-96">
                <Swiper
                  ...
                  className="mySwiper"
                >
                  {e?.images.length>0 &&e.images ? e.images.map((img)=>( <SwiperSlide key={e._id}>
                    <img src={img.image} />
              ...
              <div className="grid grid-cols-3 content-around text-gray-900 text-center">
                <div>
                  <h6>المساحه</h6>
