...
import { deleteGallery, getGallery } from "../../../../store/Gallerys/gallerySlice";

const GalleryVilla_200 = () => {
  const [isOpen, setIsOpen] = useState(false);
  const [photoIndex, setPhotoIndex] = useState(0);
  const [photoIndex, setImagelightBox] = useState(0);
              ...
              >
                {e?.images.length > 0 &&
                  e.images.map((img, idx) => (
                    <SwiperSlide
                      key={idx}
                      onClick={() => handleOpenLightbox(index * img.image.length + idx)}
                    >
