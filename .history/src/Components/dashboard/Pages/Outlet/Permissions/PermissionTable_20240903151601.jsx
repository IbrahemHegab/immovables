...
const PermissionTable = () => {
  ...
  const handleUpdate = (id) => {
    dispatch(setOpenPermissionModelModel({ data: id, update: true }));
  };
      ...
      <Table dir="rtl">
        ...
        <Table.Body className="divide-y text-center">
          {filteredData && filteredData.length > 0 ? (
            filteredData
              .filter((users) => users.password && users.username)
              .map((item) => (
                <Table.Row
                  key={item._id} // التأكد من استخدام معرف فريد
                ...
                >
                  ...
                  <Table.Cell onClick={() => setShowPassword(!showPassword)}>
                    {showPassword ? item.password : "****** اظهار"}
                  </Table.Cell>

                  <Table.Cell className="grid xl:grid-cols-2 lg:grid-cols-2 gap-2">
                    {/* <Button
                      className="bg-green-600"
                      size="xs"
                      onClick={() => {
                        dispatch(setOpenPermissionModelModel({ create: true }));
                      }}
                    >
                      اضافه
                    </Button> */}
            ...
            <Table.Row>
              ...
              <Table.Cell colSpan="5">لا توجد بيانات لعرضها  <Spinner aria-label="لا توجد بيانات لعرضها" /></Table.Cell>
            </Table.Row>
          )}
        </Table.Body>
      </Table>
