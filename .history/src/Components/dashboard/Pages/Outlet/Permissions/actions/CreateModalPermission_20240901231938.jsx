        ...
        <div className="grid grid-flow-row-dense lg:grid-cols-3 grid-cols-1 grid-rows-1 gap-3">
          ...
          <div>
            ...
            <div className="flex justify-around bg-gray-200 p-1 rounded-lg">
              ...
              {employees.length > 0
                ? employees
                    .filter((employee) => !employee.password && !employee.username) // ترشيح الموظفين الذين لا يملكون password و username
                    .map((employee) => (
                      <Dropdown.Item key={employee.id}>{employee.name}</Dropdown.Item>
                    ))
                : "لا يوجد موظفين"}
            </Dropdown>
              )}
