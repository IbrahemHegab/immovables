  ...
  async (args, thunkApi) => {
    ...
    try {
      const { data } = await axios.post(`${url}/user`, args, {
        ...
        dispatch(setOpenToast({ msg: "تم اضافة العميل بنجاح", status: true }));
        setTimeout(() => {
  ...
  async (args, thunkApi) => {
    ...
    try {
      const { data } = await axiosInstance
        ...
        .then(() => {
          dispatch(setOpenToast({ msg: "تم تعديل العميل بنجاح", status: true }));
          setTimeout(() => {
            dispatch(setOpenToast({ msg: null, status: false }));
          }, 3000);
        });
  ...
  async (args, thunkApi) => {
    ...
    try {
      const { data } = await axios.delete(`${url}/user/${args}`) .then(() => {
        dispatch(setOpenToast({ msg: "تم ح العميل بنجاح", status: true }));
        setTimeout(() => {
          dispatch(setOpenToast({ msg: null, status: false }));
        }, 3000);
      });
