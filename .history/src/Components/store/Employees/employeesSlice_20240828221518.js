...
export const deletNotifcation = createAsyncThunk(
  "employees/deleteNotifcation",
  async (args, thunkApi) => {
    try {
      const { data } = await axios.delete(`${url}/employees/notifcation/${args}`);
      return thunkApi.fulfillWithValue(data);
    } catch (e) {
      return thunkApi.rejectWithValue(e.response.data);
    }
...
const EmployeesSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(deletNotifcation.pending, (state) => {
        state.isLoading = true;
        state.error = null;
      })
      .addCase(deletNotifcation.fulfilled, (state, { payload }) => {
        state.isLoading = false;
        state.data = payload.data;
        state.itemsCount--;
        state.error = null;
      })
      .addCase(deletNotifcation.rejected, (state, { payload }) => {
        state.isLoading = false;
        state.error = payload.msg;
      });
  },
