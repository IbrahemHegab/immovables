...
export const createTask = createAsyncThunk(
  ...
  async (args, thunkApi) => {
    ...
    dispatch(setOpenToast({msg:"تم انشاء مهمة جديدة" , status}));
    setTimeout(() => {
      dispatch(setOpenToast(null))
    }, 3000);
    try {
...
export const updateTask = createAsyncThunk(
  ...
  async (args, thunkApi) => {
    const { dispatch } = thunkApi;
    dispatch(setOpenToast("تم تعديل حاله المهمة"));

    try {
      const { data } = await axios.put(`${url}/task/${args.id}`, args, {
        headers: {
          Accept: "application/json, text/plain, */*",
